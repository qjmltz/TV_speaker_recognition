    ...
    def extract_frame_by_vsf(self):
       ...
       """
        ...
        if platform.system() == 'Windows':
            ...
            Thread(target=vsf_output, args=(p.stderr,), daemon=True).start()
            p.wait()
            self.vsf_running = False
...
def ocr_subtitle_main(video_path, subtitle_area=None):
    """
    主函数入口
    :param video_path: 视频路径
    :param subtitle_area: 字幕区域，格式为(y_min, y_max, x_min, x_max)，如果为None则使用默认字幕区域
    """
    # 新建字幕提取对象
...
if __name__ == '__main__':
    ...
    video_path = '/home/z004h8mu/project_X/Project-X/ocr_subtitle/Epsode01.mp4'

    # 提示用户输入字幕区域
    try:
        # y_min, y_max, x_min, x_max = map(int, input(
        #     f"{config.interface_config['Main']['ChooseSubArea']} (ymin ymax xmin xmax)：").split()) #(330 400 240 720)
        # subtitle_area = (y_min, y_max, x_min, x_max)
        subtitle_area = None  # 默认字幕区域
        # subtitle_area = (330,400,240,720)  # 默认字幕区域
